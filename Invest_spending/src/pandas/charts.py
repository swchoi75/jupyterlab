...
def sort_data(df):
    df = df.sort_values(
        ["version", "quarter", "spending"], ascending=[True, True, False]
    )
    return df
...
def main():
    ...
    output_file = path / "report in Excel" / "Charts.xlsx"

    # Read data
    df = pd.read_csv(input_file)

    # Process data
    df = df.pipe(filter_data).pipe(summarize_data).pipe(sort_data)
