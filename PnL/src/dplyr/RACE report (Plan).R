...
main <- function() {
  ...
  input_lc <- here(
    path, "data", "RACE",
    "(Plan) Analysis FS Item Hierarchy for CU 698_LC.xlsx"
  )
  input_gc <- here(
    path, "data", "RACE",
    "(Plan) Analysis FS Item Hierarchy for CU 698_GC.xlsx"
  )
  meta_file <- here(path, "meta", "New outlet.xlsx")
  output_pnl <- here(path, "output", "(Plan) RACE Profit and Loss.csv")
  output_bs <- here(path, "output", "(Plan) RACE Balance sheet.csv")
