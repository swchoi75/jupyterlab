...
Lfcst_price <- function(df, col_name) {
  col <- col_name
  df <- df |>
    select(c(
      "Version",
    ...
    summarise(
      ...
      Lfcst_price = round(df$Sales_LC / df$Qty, 0),
    ) |>
    ungroup() |>
    tidyr::drop_na() |>
    select(!c(
    ...
    mutate(`mapping key` = df[col], .before = "Lfcst_price")
  return(df)
}
...
Lfcst_price_div_e <- function(df) {
  df <- df |>
    df_div_e() |>
    Lfcst_price("Customer Material")
  return(df)
}

Lfcst_price_div_p <- function(df) {
  df <- df |>
    df_div_p() |>
    Lfcst_price("Product")
  return(df)
...
Lfcst_price_pl_cm <- function(df) {
  df <- df |>
    df_pl_cm() |>
    Lfcst_price("CM Cluster")
  return(df)
}

# mapping between between actual and forecast ----
spv_mapping <- function(df, Lfcst_price, col_name) {
  df <- df |>
    left_join(Lfcst_price, by = c("Profit Ctr", col_name))
  return(df)
}
